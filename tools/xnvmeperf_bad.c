...
/**
 * Async IO completion callback (submit-on-completion strategy).
 *
 * Updates job counters and immediately resubmits the next IO on the same ctx,
 * inspired by the SIL (Submit-In-Latency) pattern. The ctx is only returned to
 * the queue on submission failure.
 *
...

	{
		...
		while (1) {
			for (int i = 0; i < thread->ndevs; i++) {
				...
				// Completions trigger cb_fn which resubmits immediately
